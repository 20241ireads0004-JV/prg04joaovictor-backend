...
public class AtletaService implements AtletaIService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(AtletaService.class);
    ...
    public Atleta save(Atleta atleta) {
        ...
        logger.info(
                "[SERVICE] Iniciando cadastro do atleta: {}",
                atleta.getLogin()
        );
        ...
        try {
            ...
            logger.info(
                    "[SERVICE] Atleta cadastrado com sucesso."
            );
            ...
            logger.error(
                    "[SERVICE] Erro ao salvar atleta: {}",
                    e.getMessage()
            );

            throw new BusinessException(
                    "Erro interno ao realizar cadastro do atleta."
            );
        }
    }
    ...
    public Page<Atleta> findAll(Pageable pageable) {
        ...
        logger.info(
                "[SERVICE] Buscando todos os atletas."
        );
    ...
    public void delete(Long id) {
        ...
        logger.info(
                "[SERVICE] Solicitação de exclusão do atleta ID: {}",
                id
        );
        ...
        if (!atletaRepository.existsById(id)) {
            ...
            logger.error(
                    "[SERVICE] Atleta não encontrado. ID: {}",
                    id
            );
        ...
        logger.info(
                "[SERVICE] Atleta deletado com sucesso."
        );
    }
    ...
    public Atleta update(Long id, Atleta atleta) {
        ...
        logger.info(
                "[SERVICE] Atualizando atleta ID: {}",
                id
        );

        Atleta atletaExistente =
                atletaRepository.findById(id)
                        .orElseThrow(() ->
                                new ResourceNotFoundException(
                                        "Atleta não encontrado."
                                )
        ...
        if (atletaRepository.existsByEmailAndIdNot(
                atleta.getEmail(),
                id
        )) {
        ...
        if (atletaRepository.existsByLoginAndIdNot(
                atleta.getLogin(),
                id
        )) {
        ...
        atletaExistente.setEstatistica(
                atleta.getEstatistica()
        );

        atletaExistente.setEquipes(
                atleta.getEquipes()
        );

        atletaExistente.setGruposEsportivos(
                atleta.getGruposEsportivos()
        );

        Atleta atletaAtualizado =
                atletaRepository.save(atletaExistente);

        logger.info(
                "[SERVICE] Atleta atualizado com sucesso."
        );
    ...
    private void validarDadosUnicos(
        ...
        if (atletaRepository.existsByEmail(
                atleta.getEmail()
        )) {
        ...
        if (atletaRepository.existsByLogin(
                atleta.getLogin()
        )) {
